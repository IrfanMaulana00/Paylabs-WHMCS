...
class Paylabs_VtWeb
{
  ...
  public static function getPriceByOrderId($orderId) {
    $command = 'GetInvoice';
    $postData = array(
        'invoiceid' => $orderId,
    );
    ...
    if($invoice['result'] == 'success') {
        // Extract price from invoice details
        $price = $invoice['total'];
        return $price;
    } else {
        return false; // Order ID not found or invoice doesn't exist
    }
}
  ...
  public static function validateTransaction($publicKey, $signature, $dataToSign, $date)
  {
    ...
    $signatureAfter = "POST:/whmcs/modules/gateways/callback/paylabs_callback.php:" . $shaJson . ":" . $date;
